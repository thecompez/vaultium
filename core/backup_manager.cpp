...
auto publishNoClobber(
    const std::filesystem::path& temporaryFile,
    const std::filesystem::path& finalFile
) -> void
{
#if defined(VAULTIUM_PLATFORM_LINUX) || defined(VAULTIUM_PLATFORM_MACOS)
    if (::link(temporaryFile.c_str(), finalFile.c_str()) != 0) {
        if (errno == EEXIST) {
            throw std::runtime_error("Refusing to overwrite an existing artifact: " + finalFile.string());
        }
        throw std::runtime_error(
            "Could not publish artifact " + finalFile.string() + ": " + std::strerror(errno)
        );
    }

    std::error_code error;
    if (!std::filesystem::remove(temporaryFile, error)) {
        std::filesystem::remove(finalFile, error);
        throw std::runtime_error("Could not remove temporary artifact after publication: " + temporaryFile.string());
    }
#else
    static_cast<void>(temporaryFile);
    static_cast<void>(finalFile);
    throw std::runtime_error("No-clobber publication is not implemented for this platform yet.");
#endif
}

auto publishTextSidecar(
    const std::filesystem::path& finalPath,
    const std::string& content
) -> void
{
    const auto temporaryPath = std::filesystem::path { finalPath.string() + ".tmp" };
    {
        std::ofstream file { temporaryPath, std::ios::trunc };
        if (!file) {
            throw std::runtime_error("Could not write sidecar: " + finalPath.string());
        }
        file << content;
        file.close();
        if (!file) {
            ...
            throw std::runtime_error("Could not finalize sidecar: " + finalPath.string());
        }
    }
    ...
    try {
        publishNoClobber(temporaryPath, finalPath);
    } catch (...) {
        std::error_code error;
        std::filesystem::remove(temporaryPath, error);
        throw;
...
auto BackupManager::runOnce() -> void
{
    ...
    try {
        ...
        publishNoClobber(artifact.path, finalFile);
        published = true;
...
auto BackupManager::writeChecksumSidecar(const std::filesystem::path& artifact) const -> std::string
{
    ...
    publishTextSidecar(
        sidecar,
        checksum + "  " + artifact.filename().string() + "\n"
    );
    return checksum;
}
...
) const -> void
{
    ...
    publishTextSidecar(sidecarPath(artifact, ".meta.json"), content.str());
}
